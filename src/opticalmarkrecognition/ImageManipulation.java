                ...
                for(int i = 0; i < line.length(); i++) {
                    ...
                    if(ch != '-' && ch != '0') {
                        ...
                        try{
                            ...
                            System.out.println("Error found"+m+" ; "+n+" ; "+ ascTemplate[m][n]+ ";" + realMarkLocations[ascTemplate[m][n]]);
                        }
...
//                    }
    ...
    public void searchMarks(String projName) {
        String first="";
        String second="";
        ...
        for(int i = 0; i < realNummarks; i++) {
            ...
            if(mark.isMark(x, y)) {
                ...
                if(j==1){
                first=field.getName();
                second=field.getValue(i);
                String qry="INSERT INTO "+projName+" ("+field.getName()+") VALUES('"+field.getValue(i)+"')";
                dbh.testing(qry);
                String qryid = "SELECT * FROM "+projName;
                String lastidd = dbh.getLastId(qryid);
                lastid = Integer.parseInt(lastidd);
                System.out.println(lastid);
                j++;
                }else{
                String qry1 = "SELECT * FROM "+projName+" WHERE id = "+lastid;
                String val = dbh.processQRY(qry1, field.getName());
                System.out.println(field.getName()+" "+val);
                if(val==null){
                    String qry = "UPDATE "+projName+" SET "+field.getName()+" = '"+field.getValue(i)+"' WHERE id = "+lastid;
